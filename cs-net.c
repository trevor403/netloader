...
bool CSNetDecap(CSSocket* s,void* data,void* dbuffer,u32* length)
{
    ...
    GRAPH_Rectangle(0,450,350,30,COLOR_BLACK);
    DEBUG_PrintChar(0,450,'K');
    DEBUG_ShowValueU32(50, 450, type);
    ...
    switch (type)
    {
      case 0x00: // IP 
        ...
        if (memcmp(&ip->dstIP,&s->localIP,4) && s->localIP != 0x00000000)
        {
            *length = 0;
            return false;
        }
        ...
        GRAPH_Rectangle(0,290,350,30,COLOR_BLACK);
        DEBUG_PrintChar(0,290,'d');
        DEBUG_ShowValueU32(50, 290, ip->dstIP);
      ...
      case 0x06: // ARP 
        GRAPH_Rectangle(0,290,350,30,COLOR_BLACK);
        DEBUG_PrintChar(0,290,'x');
        DEBUG_ShowValueU32(50, 290, 0x9999);
...
int CSNetARPAddress(u48 localMAC,u48* remoteMAC,u32 localIP,u32 remoteIP)
{
    ...
    DEBUG_ShowValueU32(0, 0, remoteIP);
...
void CSNetDoDHCP()
{
    ...
    *(u32*)((u32)(&bufferout[0]) + 0xEC) = 0x63825363;
    ...
    //CSNetInitReceive();
    u32 skips = 0;
    for (u32 count = 0;; count++) // CSNetReceiveFrom(CS_PACKET_buffer,0,CONTROLSIM_PORT)
    {

        GRAPH_Rectangle(0,260,320,30,COLOR_BLACK);
        DEBUG_PrintChar(0,260,'c');
        DEBUG_ShowValueU32(50, 260, count);

        CSNetInitReceive();
        if (CS_ENTITY->bufferSize == 0) {
          skips++;
          GRAPH_Rectangle(0,320,320,30,COLOR_BLACK);
          DEBUG_PrintChar(0,320,'z');
          DEBUG_ShowValueU32(50, 320, skips);
          continue;
        }
        ...
        GRAPH_Rectangle(0,320,320,30,COLOR_BLACK);
        DEBUG_ShowValueU8(0, 320, 0x40);

        if (*(u32*)((u32)(&bufferin[0]) + 4) != transID) continue;

        GRAPH_Rectangle(0,320,320,30,COLOR_BLACK);
        DEBUG_ShowValueU8(0, 320, 0x80);
    ...
    *(u32*)((u32)(&bufferout[0]) + 0xEC) = 0x63825363;
    ...
    DEBUG_PrintChar(0,400,'e');
    DEBUG_ShowValueU32(50,400, defGW);
